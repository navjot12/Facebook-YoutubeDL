...
def scraper(search):
	...
	for video in videos:
		...
		try:
			...
			views = data.find('div', {'class': 'yt-lockup-meta'}).find('ul').find_all('li')[1].text.decode('utf-8', 'strict')
			image = thumbnail.find('span', {'class': 'yt-thumb-simple'}).find('img')['src'].decode('utf-8', 'strict')
